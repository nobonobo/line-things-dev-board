const USER_SERVICE_UUID = "a0bbce01-19d2-44c3-88b4-313f3ac87599";
...
async function checkAvailablityAndDo(callbackIfAvailable) {
  ...
  if (isAvailable) {
    document.getElementById("alert-liffble-notavailable").style.display =
      "none";
    callbackIfAvailable();
  } else {
    document.getElementById("alert-liffble-notavailable").style.display =
...
function addDeviceToList(device) {
  ...
  const deviceItem = document
    .getElementById("device-list-item")
    .cloneNode(true);
...
function connectDevice(device) {
  ...
  if (!device) {
    ...
    connectingUUIDSet.has(device.id) ||
    connectedUUIDSet.has(device.id)
  ) {
...
function initializeCardForDevice(device) {
  ...
  template.querySelector(".card > .card-header > .device-name").innerText =
    device.name;

  //LED Control
  template.querySelector(".led0_value").addEventListener("change", () => {
    onScreenLog("LED0 Value changed");
    updateLedState(device).catch(e =>
      onScreenLog(`ERROR on updateLedState(): ${e}\n${e.stack}`)
    );
  ...
  template.querySelector(".setuuid").addEventListener("click", () => {
    writeAdvertuuid(device, template.querySelector(".uuid_text").value).catch(
      e => onScreenLog(`ERROR on writeAdvertuuid(): ${e}\n${e.stack}`)
    );
  });

  template
    .querySelector(".notification-enable")
    .addEventListener("click", () => {
      toggleNotification(device).catch(e =>
        onScreenLog(`ERROR on toggleNotification(): ${e}\n${e.stack}`)
      );
    });
...
function updateConnectionStatus(device, status) {
  if (status == "connected") {
    ...
    statusBtn.setAttribute(
      "class",
      "device-status btn btn-outline-primary btn-sm disabled"
    );
    ...
    statusBtn.setAttribute(
      "class",
      "device-status btn btn-outline-secondary btn-sm disabled"
    );
    ...
    statusBtn.setAttribute(
      "class",
      "device-status btn btn-outline-danger btn-sm disabled"
    );
...
async function toggleNotification(device) {
  ...
  const accelerometerCharacteristic = await getCharacteristic(
    device,
    USER_SERVICE_UUID,
    USER_CHARACTERISTIC_NOTIFY_UUID
  );
  ...
  if (notificationUUIDSet.has(device.id)) {
    ...
    getDeviceNotificationButton(device)
...
async function stopNotification(characteristic, callback) {
  ...
  onScreenLog(
    "Notifications STOPPED　" + characteristic.uuid + " " + device.id
  );
}
...
async function refreshValues(device) {
  ...
  const accelerometerBuffer = await readCharacteristic(
...
async function getCharacteristic(device, serviceId, characteristicId) {
  ...
  onScreenLog(
    `Got characteristic ${serviceId} ${characteristicId} ${device.id}`
  );
...
function getDeviceProgressBarTemperature(device) {
  return getDeviceCard(device).getElementsByClassName(
